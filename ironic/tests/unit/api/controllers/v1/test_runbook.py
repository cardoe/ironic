...
class TestRunbookTraitsController(BaseRunbooksAPITest):
    """Tests for /v1/runbooks/{ident}/traits (v1.112+)."""
...
class TestRunbookV112(BaseRunbooksAPITest):
    ...
    def test_create_with_traits_v112(self):
        """Runbooks created via v1.112 may supply an initial traits list."""
        tdict = test_api_utils.post_get_test_runbook(name='my-traits-rb')
        tdict['traits'] = ['CUSTOM_FOO', 'CUSTOM_BAR']
        response = self.post_json('/runbooks', tdict, headers=V112_HEADERS)
        self.assertEqual(http_client.CREATED, response.status_int)
        # Traits should be reflected in the response
        self.assertCountEqual(['CUSTOM_FOO', 'CUSTOM_BAR'],
    ...
    def test_get_one_includes_traits_v112(self):
        runbook = obj_utils.create_test_runbook(self.context)
        objects.RunbookTraitList.create(
            self.context, runbook.id, ['CUSTOM_A'])
        data = self.get_json('/runbooks/%s' % runbook.uuid,
                             headers=V112_HEADERS)
