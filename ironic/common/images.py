...
"""
Handling of VM disk images using pathlib for path operations.
"""
...
def _create_root_fs(root_directory: Union[str, Path],
                    files_info: Dict[Union[str, bytes, Path], str]) -> None:
    """Creates a filesystem root in given directory using pathlib.
    ...
    :param root_directory: the filesystem root directory as Path or string.
    ...
    for src_file, path in files_info.items():
        ...
        target_file = root_path / path

        if target_file.parent != root_path:
...
def create_vfat_image(output_file: Union[str, Path],
                      files_info: Optional[Dict[Union[str, bytes, Path], str]] = None,
                      parameters: Optional[Dict[str, str]] = None,
                      parameters_file: str = 'parameters.txt',
                      fs_size_kib: int = 100) -> None:
    """Creates the fat fs image on the desired file using pathlib.
    ...
    with utils.tempdir() as tmpdir:
        ...
        try:
            if files_info:
                _create_root_fs(tmpdir_path, files_info)

            if parameters:
                params_file_path = tmpdir_path / parameters_file
            ...
            file_list_str = [str(item) for item in file_list]
...
def _generate_cfg(kernel_params: Optional[List[str]],
                  template: Union[str, Path],
                  options: Dict[str, str]) -> str:
    """Generates a isolinux or grub configuration file using pathlib.
    ...
    :param template: the path of the config template file (Path or string).
...
def _label(files_info: Dict[Union[str, bytes, Path], str]) -> str:
    """Get a suitable label for the files.

    Returns "config-2" if the openstack metadata is present.

    :param files_info: A dict containing file paths or content to relative paths.
    :returns: Label string for the ISO image.
    """
...
def create_isolinux_image_for_bios(
        ...
        ramdisk: Union[str, Path],
        kernel_params: Optional[List[str]] = None,
        inject_files: Optional[Dict[Union[str, Path], str]] = None,
        publisher_id: Optional[str] = None) -> None:
    """Creates an isolinux image on the specified file using pathlib.
    ...
    :param output_file: the path to the file where the iso image needs to be
        created (Path or string).
    :param kernel: the kernel to use (Path or string).
    :param ramdisk: the ramdisk to use (Path or string).
    :param kernel_params: a list of strings(each element being a string like
    ...
    with utils.tempdir() as tmpdir:
        ...
        ldlinux_src: Optional[Union[str, Path]] = None
        if CONF.ldlinux_c32:
            ldlinux_src = CONF.ldlinux_c32
        else:
            for directory in LDLINUX_SRC_DIRS:
                ldlinux_src_path = Path(directory) / 'ldlinux.c32'
                if ldlinux_src_path.is_file():
                    ldlinux_src = ldlinux_src_path
                    break
            else:
                ldlinux_src = None
        if ldlinux_src:
        ...
        try:
            utils.execute('mkisofs', '-r', '-V', _label(files_info),
                          ...
                          '-b', ISOLINUX_BIN, '-o', str(output_file), str(tmpdir_path))
        except processutils.ProcessExecutionError as e:
...
def create_esp_image_for_uefi(
        ...
        kernel: Union[str, Path],
        ramdisk: Union[str, Path],
        deploy_iso: Optional[Union[str, Path]] = None,
        esp_image: Optional[Union[str, Path]] = None,
        kernel_params: Optional[List[str]] = None,
        inject_files: Optional[Dict[Union[str, Path], str]] = None,
        publisher_id: Optional[str] = None) -> None:
    """Creates an ESP image on the specified file using pathlib.
    ...
    :param deploy_iso: deploy ISO image to extract EFI system partition image
        from (Path or string). If not specified, the `esp_image` option is required.
    :param esp_image: FAT12/16/32-formatted EFI system partition image
        ...
        architecture to boot (Path or string). This image will be embedded into the ISO image.
    ...
    with utils.tempdir() as tmpdir:
        ...
        files_info: Dict[Union[str, bytes, Path], str] = {
            kernel: 'vmlinuz',
            ramdisk: 'initrd',
        }
        if inject_files:
...
def fetch_into(context: Any,
               image_href: str,
               image_file: Union[str, Path, IO[bytes]],
               image_auth_data: Optional[Dict[str, Any]] = None) -> Optional[str]:
    """Fetches image file contents into a file using pathlib.

    :param context: A context object.
    :param image_href: The Image URL or reference to attempt to retrieve.
    :param image_file: The file handler or file name (Path or string) to write
                       the requested file contents to.
...
def _handle_zstd_compression(path: Union[str, Path]) -> None:
    """Handles zstd compression detection and decompression using pathlib.

    :param path: Path to the file to check and potentially decompress (Path or string).
    """
    file_path = Path(path)
    zstd_comp = False

    with file_path.open('rb') as comp_check:
    ...
    if zstd_comp and not CONF.conductor.disable_zstandard_decompression:
        temp_path = file_path.with_suffix(file_path.suffix + '.zstd')
...
def fetch(context: Any,
    ...
    """Fetches an image and optionally converts to raw format using pathlib.

    :param context: A context object.
    :param image_href: The Image URL or reference to attempt to retrieve.
    :param path: Path where the image should be saved (Path or string).
    :param force_raw: Whether to convert the image to raw format.
    :param checksum: Optional checksum to validate against.
    :param checksum_algo: Optional checksum algorithm.
    :param image_auth_data: Optional authentication data for image download.
    """
    ...
    if force_raw:
        path_obj = Path(path)
        image_to_raw(image_href, path, path_obj.with_suffix('.part'))


def detect_file_format(path: Union[str, Path]) -> Any:
    """Re-implementation of detect_file_format from oslo.utils using pathlib.

    This implementation specifically allows for single multi-format
    combination of ISO+GPT, which it treats like ISO.

    :param path: Path to the image file to detect (Path or string).
...
def get_source_format(image_href: str, path: Union[str, Path]) -> str:
    """Gets the source format of an image file using pathlib.

    :param image_href: The image reference for logging.
    :param path: Path to the image file (Path or string).
    :returns: String representation of the image format.
    """
    try:
...
def force_raw_will_convert(image_href: str, path_tmp: Union[str, Path]) -> bool:
    """Determines if force_raw will require conversion using pathlib.

    :param image_href: The image reference for logging.
    :param path_tmp: Path to the temporary image file (Path or string).
...
def image_to_raw(image_href: str,
                 path: Union[str, Path],
                 path_tmp: Union[str, Path]) -> None:
    """Converts an image to raw format if needed using pathlib.

    :param image_href: The image reference for logging.
    :param path: Path where the final image should be saved (Path or string).
    :param path_tmp: Path to the temporary image file (Path or string).
    """
    path_obj = Path(path)
    ...
    with fileutils.remove_path_on_error(str(path_tmp_obj)):
        ...
        if fmt not in RAW_IMAGE_FORMATS and fmt != "iso":
            ...
            staged = path_obj.with_suffix('.converted')
...
def image_show(context: Any,
    ...
    :param context: A context object.
    :param image_href: The image reference.
    :param image_service: Optional image service instance.
    :param image_auth_data: Optional authentication data.
...
def download_size(context: Any,
                  image_href: str,
                  image_service: Optional[Any] = None,
                  image_auth_data: Optional[Dict[str, Any]] = None) -> int:
    """Gets the download size of an image.
...
def converted_size(path: Union[str, Path], estimate: bool = False) -> int:
    """Get size of converted raw image using pathlib.
...
def get_image_properties(context: Any,
                         image_href: str,
...
def create_boot_iso(context: Any,
                    ...
                    inject_files: Optional[Dict[Union[str, Path], str]] = None,
                    publisher_id: Optional[str] = None) -> None:
    """Creates a bootable ISO image for a node using pathlib.
    ...
    :param context: context
    :param output_filename: the absolute path of the output ISO file (Path or string)
    ...
    with utils.tempdir() as tmpdir:
        ...
        if boot_mode == 'uefi':
            ...
            deploy_iso_path: Optional[Union[str, Path]] = None
            esp_image_path: Optional[Union[str, Path]] = None
...
def _extract_iso(extract_iso: Union[str, Path],
                 extract_dir: Union[str, Path]) -> None:
    """Extracts an ISO image to a directory using pathlib.

    :param extract_iso: Path to the ISO file to extract (Path or string).
    :param extract_dir: Path to the directory to extract to (Path or string).
    """
    ...
    for dirname, dirlist, filelist in iso.walk(iso_path='/'):
        ...
        for file in filelist:
            file_path = extract_dir_path / dirname / file
...
def _get_deploy_iso_files(deploy_iso: Union[str, Path],
                           mountdir: Union[str, Path]) -> Tuple[Dict[str, str], str, str]:
    """This function opens up the deploy iso used for deploy using pathlib.

    :param deploy_iso: path to the deploy iso where its contents are fetched to
                       (Path or string).
    :param mountdir: path to the mount directory (Path or string).
    ...
    try:
        _extract_iso(deploy_iso, mountdir_path)
    except Exception as e:
    ...
    uefi_path_info = {str(e_img_path): e_img_rel_path,
                      str(grub_path): grub_rel_path}
...
def __node_or_image_cache(node: Optional[Any]) -> str:
    ...
    :param node: Optional node object.
    :returns: 'image cache' if node is None, otherwise node.uuid.
    """
    if not node:
        return 'image cache'
    else:
...
def safety_check_image(image_path: Union[str, Path],
                       node: Optional[Any] = None) -> str:
    """Performs a safety check on the supplied image using pathlib.
    ...
    :param image_path: A fully qualified path to an image which needs to
                       be evaluated for safety (Path or string).
...
def image_format_permitted(img_format: str) -> bool:
    """Checks if an image format is permitted.

    :param img_format: The image format to check.
    :returns: True if the format is permitted, False otherwise.
    """
...
def image_format_matches(actual_format: str, expected_format: str) -> bool:
    """Checks if actual format matches expected format.

    :param actual_format: The actual detected format.
    :param expected_format: The expected format.
    :returns: True if formats match, False otherwise.
    """
...
def check_if_image_format_is_permitted(img_format: str,
