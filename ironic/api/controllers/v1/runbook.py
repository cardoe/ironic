...
# a 'traits' list is accepted.
RUNBOOK_SCHEMA_V112 = {
    'type': 'object',
    'properties': {
        'uuid': {'type': ['string', 'null']},
        'name': RUNBOOK_NAME_SCHEMA,
...
# Schema used when validating PATCH bodies for v1.112+.  Traits are not
# patchable via this endpoint (use the /traits sub-resource instead), so
# 'traits' is intentionally absent from the properties here.
RUNBOOK_PATCH_SCHEMA_V112 = {
    'type': 'object',
    'properties': {
        'uuid': {'type': ['string', 'null']},
        'name': RUNBOOK_NAME_SCHEMA,
    ...
    def put(self, trait=None, body=None):
        ...
        if trait:
            if api.request.body and api.request.json_body:
                ...
                raise exception.Invalid(msg)
            new_traits = set(rpc_runbook.traits) | {trait}
            objects.RunbookTraitList.create(
                context, rpc_runbook.id, sorted(new_traits))
...
class RunbooksController(rest.RestController):
    ...
    def _lookup(self, runbook_ident, *remainder):
        if remainder and remainder[0] == 'traits':
    ...
    def post(self, runbook):
        ...
        # For pre-v1.112 requests, traits are not in the schema so they won't
        # be present. For v1.112+ they may be provided.
        if not api_utils.allow_runbook_traits():
            runbook.pop('traits', None)
    ...
    def patch(self, runbook_ident, patch=None):
        ...
        # Use the patch-specific schema (no traits field) so that
        # patch_update_changed_fields does not try to overwrite traits.
        if api_utils.allow_runbook_traits():
            patch_schema = RUNBOOK_PATCH_SCHEMA_V112
        else:
            patch_schema = RUNBOOK_SCHEMA
